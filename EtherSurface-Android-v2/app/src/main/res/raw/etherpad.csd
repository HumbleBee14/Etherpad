...
<CsScore>
; $INF defines an effectively-infinite duration (~100 hours). The legacy
; 2014 score used $INF as a bare token that Csound 5.x silently treated as
; a large number; Csound 6.x expands undefined macros to empty string,
; which truncates lines like "i888 0 $INF" to "i888 0" → 0 duration →
; every always-on instrument deallocates immediately → score ends after
; one ksmps → csoundPerformKsmps returns 2 forever → silence.
#define INF # 360000 #
i888 0 $INF
i999 0 $INF
i"Mixer" 0 $INF
